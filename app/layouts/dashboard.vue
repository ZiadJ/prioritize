...
const sidebarCollapsedByClick = ref(false)
...
function conditionalSidebarHover(e) {
	if (sidebarCollapsedByClick.value) return
	handleSidebarHover(e)
}

function collapseSidebar() {
	sidebarCollapsedByClick.value = true
...
function resetSidebarCollapse() {
	sidebarCollapsedByClick.value = false
	sidebarExpanded.value = false
}
...
class="bg-white dark:bg-zinc-900 h-full rounded-lg shadow-md pt-3 pb-4 px-0"
		...
		<div class="flex flex-col w-full h-screen bg-surface-ground">
			...
			@mousemove="conditionalSidebarHover"
			@mouseout="handleSidebarHover"
			@mouseleave="resetSidebarCollapse"
