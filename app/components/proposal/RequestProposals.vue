...
import { RequestNode, ProposalNode } from '@/db/models'
...
const jsonData = computed({
  ...
  set: (newValue) => {
    requestNodes.value = JSON.parse(newValue).data as RequestNode[]
    const id = props.requestId

    requestNode.value = requestNodes.value.find((req) => req.id === id)

    if (requestNode.value) {
      rootNode.value = utils.tree
        .buildTree(requestNode.value.children)
        .map<TreeNodeEx>((node) => {
          return {}
        })
      ...
      if (proposals)
        columnsInit
          .concat(proposals.map((p: ProposalNode) => mapProposalToColumn(p)))
          .forEach((col) => columns.value.push(col)) // value cannot be set directly so using push instead
    }
  }
})
...
const selectedNode = ref<TreeNodeEx>({ data: {} })
...
function addProposal(proposal: ProposalNode | null = null) {
  if (!proposal) {
    proposal = {
      id: -columns.value.length,
      title: prompt('Title for the new proposal', 'New Proposal') || '',
      description:
        prompt('Description for the proposal', 'New Description') || '',
      isActive: true
    } as ProposalNode
  }
...
function mapProposalToColumn(p: ProposalNode): ProposalColumn {
  return {
    field: 'p' + p.id,
    columnKey: 'p' + p.id,
    header: p.title,
...
function mapColumnToProposal(p: ProposalColumn): ProposalNode {
  return {
...
const visibleColumns = ref<TreeNodeEx[]>(columns.value)
...
const getParentArray = (rootNode: TreeNodeEx, node: TreeNodeEx): TreeNodeEx => {
  return (
    treeUtils.traverseTreeUntil<TreeNodeEx>(rootNode, (child) => {
      return child === node
      ...
      <Splitter style="height: 300px">
        ...
        <Splitter style="height: 85px">
          ...
          <SplitterPanel>
            ...
            <Editor
              :readonly="true"
              v-model="hoveredProposal.description"
              editorStyle="height: 80px; font-size: 14px;"
              :autoResize="true"
            />
          </SplitterPanel>
